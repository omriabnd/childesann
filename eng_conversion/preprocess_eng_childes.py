...
def to_conll(morphdep, debug=False):
    ...
    for index, word in enumerate(morphdep):
        ...
        if pos in ['+...', '+/.']:
            ...
            added_indices.append((index + offset, 1,
                                  0))  # (original token number after which the addition occured, how many tokens were added, which of them is the head (0,1,2 etc.))
            offset += 1
            ...
            if len(sub_words) != 3 or sub_words[1] != 'ha':
                ...
                output.append(
                    [index + offset + 1, sub_words[1], sub_words[1], 'det', 'det', '_', index + offset + 2, 'det', '_',
                     '_'])
                output.append(
                    [index + offset + 2, sub_words[2], sub_words[2], 'n', 'n', '_', index + offset, 'nmod', '_', '_'])
