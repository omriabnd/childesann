...
"""
Modifications done for the Hebrew CHILDES conversion script by Omri Abend.

1. run_depedit now uses 'yield' instead of returning a string in the end of the process.
2. Note: the transofrmations are applied in the order they are listed in the config file.
"""
...
def process_sentence(conll_tokens, tokoffset, transformations):
    for transformation in transformations:
        ...
        if len(result_sets) > 0:
            # DEBUG BLOCK
            #if transformation.relations != ['none']:
            #        pdb.set_trace()
            # END OF DEBUG BLOCK
        ...
        if len(result) > 0:
            for action in actions:
                if ":" in action:  # Unary node instruction
                    ...
                    # Omri Abend added this 11/7
                    if re.match(r'%[0-9]+',value):
                        source_node_position = int(value[1:])
                        value = getattr(result[source_node_position],property)
                    # End of block by Omri Abend
...
def run_depedit(infile, config_file):
    ...
    for myline in infile:
        if sentlength > 0 and "\t" not in myline:
            yield 'Sentence', process_sentence(conll_tokens,tokoffset,transformations)
            sentence_string = ""
            if sentlength > 0:
                tokoffset += sentlength
            sentlength = 0
        if myline.startswith("#"):  # Preserve comment lines
            yield 'Comment', myline
        elif myline.strip() == "":
            yield 'Space', "\n"
